...
deaths.rate = [isnothing(v) || ismissing(v) ? missing : v isa Float64 ? v : tryparse(Float64, string(v)) for v in deaths.rate]
...
function norm_to_percents(unnormed)
    return unnormed / sum(unnormed)
end
